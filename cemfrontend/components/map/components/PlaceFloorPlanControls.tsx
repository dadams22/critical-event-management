...
export default function PlaceFloorPlanControls({
	...
	useEffect(() => {
		...
		const computeRect = (center: [number, number], size: [number, number]) => {
			const cUL = map.unproject([center[0] - size[0] / 2, center[1] - size[1] / 2]).toArray();
			const cUR = map.unproject([center[0] + size[0] / 2, center[1] - size[1] / 2]).toArray();
			const cLR = map.unproject([center[0] + size[0] / 2, center[1] + size[1] / 2]).toArray();
			const cLL = map.unproject([center[0] - size[0] / 2, center[1] + size[1] / 2]).toArray();
